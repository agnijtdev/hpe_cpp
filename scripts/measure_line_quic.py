...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 scripts/measure_line_quic.py <number_of_routers> <trials>")
        print("Example: python3 scripts/measure_line_quic.py 10 10")
        sys.exit(1)
    ...
    output_file = output_dir / f"line_quic_{n}_routers.csv"

    print(f"Measuring QUIC BGP convergence from {source_router} to {target_router}")
    ...
    if not route_exists(target_router):
        ...
        print("Run ./scripts/start_quic_line_lab.sh 10 first.")
        sys.exit(1)
