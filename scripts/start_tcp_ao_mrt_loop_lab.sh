...
BASE_DIR="$PROJECT_DIR/generated_configs/tcp_ao_mrt_loop_${N}"
RESULT_DIR="$PROJECT_DIR/results/mrt_tcp_ao_loop_${N}_${PREFIX_COUNT}_announce${DELAY_MS}_delay${LINK_DELAY_MS}"
PREFIX_FILE="$BASE_DIR/exabgp/prefixes.txt"

mkdir -p "$BASE_DIR" "$BASE_DIR/exabgp" "$RESULT_DIR"

network_name() {
  echo "bgpost_mrt_tcp_ao_link_$1"
}

subnet_for_link() {
  echo "172.40.$((10 + $1)).0/24"
}
...
INJ_NET="bgpost_mrt_tcp_ao_injector"
INJ_SUBNET="172.40.0.0/24"
EXABGP_IP="172.40.0.10"
R1_EXABGP_IP="172.40.0.2"

RETURN_NET="bgpost_mrt_tcp_ao_return"
RETURN_SUBNET="172.40.20.0/24"
R10_RETURN_IP="172.40.20.11"
R1_RETURN_IP="172.40.20.12"
...
docker network ls --format '{{.Name}}' | grep '^bgpost_mrt_tcp_ao_' | xargs -r docker network rm >/dev/null || true
...
echo
echo "TCP-AO MRT loop experiment complete."
