...
CERT_DIR="$PROJECT_DIR/certs/quic_line_${N}"
CONFIG_DIR="$PROJECT_DIR/generated_configs/quic_line_${N}"
GEN_CERT="$PROJECT_DIR/external/BGPoST-Artifacts/cert-generator/generate_certs.sh"

network_name() {
  echo "bgpost_quic_line_net_$1"
}

subnet_for_link() {
  echo "172.35.$((10 + $1)).0/24"
}
...
echo "[1/9] Generating QUIC BIRD configs..."
python3 "$PROJECT_DIR/scripts/generate_quic_line_configs.py" "$N"

echo "[2/9] Cleaning old QUIC line containers/networks..."
for i in $(seq 1 50); do
  docker rm -f "r$i" >/dev/null 2>&1 || true
done

docker network ls --format '{{.Name}}' | grep '^bgpost_quic_line_net_' | xargs -r docker network rm >/dev/null || true
...
echo "[5/9] Starting QUIC router containers..."
for i in $(seq 1 "$N"); do
  if [ "$i" -eq 1 ]; then
...
echo "[9/9] Waiting for all QUIC BGP sessions..."
for link in $(seq 1 $((N - 1))); do
  left_router="r${link}"
...
docker exec "r$N" ss -unlp | grep 179 || true

echo
echo "QUIC line lab started successfully."
