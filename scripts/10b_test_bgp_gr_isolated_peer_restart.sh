...
OUT="evidence/bgp_gr_llgr/bgp_gr_isolated_peer_restart_${TS}.txt"
PING_LOG="evidence/bgp_gr_llgr/ping_h1_to_h3_isolated_${TS}.log"
ROUTEGET_LOG="evidence/bgp_gr_llgr/route_get_isolated_${TS}.log"
BIRD_ROUTE_LOG="evidence/bgp_gr_llgr/bird_route_isolated_${TS}.log"
BGP_STATE_LOG="evidence/bgp_gr_llgr/bgp_state_isolated_${TS}.log"

CSV="results/bgp_gr_llgr/bgp_gr_isolated_peer_restart_${TS}.csv"
CSV_LATEST="results/bgp_gr_llgr/bgp_gr_isolated_peer_restart.csv"
...
cleanup() {
  echo
  echo "Cleanup: enabling hpe-r1 protocol r2 again..."
...
{
  echo "Isolated BGP GR / LLGR Peer Restart Test"
  echo "Date: $(date)"
  echo
  echo "Goal:"
  echo "Prove forwarding continuity without using the alternate hpe-r1 -> hpe-r2 BGP path."
  echo
  echo "Traffic under test: hpe-h1 -> hpe-h3"
  echo "Target network: $TARGET_NET"
  echo "Target IP: $TARGET_IP"
  echo
  echo "Direct BGP next-hop under test: $OLD_NH"
  echo "Temporarily disabling alternate iBGP protocol on hpe-r1: r2"
  echo

  echo "============================================================"
  echo "1. Disable alternate BGP path hpe-r1 -> hpe-r2"
  echo "============================================================"
  ...
  echo
  echo "---- hpe-r1 BGP protocol summary after disabling r2 ----"
  docker exec hpe-r1 birdc show protocols | grep -Ei "r9|r2|BGP|Established|disabled|down|start|Active" || true

  echo
  echo "---- hpe-r1 route to $TARGET_NET after disabling r2 ----"
  docker exec hpe-r1 birdc show route "$TARGET_NET" all || true

  echo
  echo "---- hpe-r1 route-get to $TARGET_IP after disabling r2 ----"
  docker exec hpe-r1 ip route get "$TARGET_IP" || true

  echo
  echo "Expected now: only direct route via $OLD_NH should be usable."
  echo

  echo "---- Baseline ping with alternate path disabled ----"
  ...
  echo
  echo "============================================================"
  echo "3. Restart direct BGP peer session hpe-r9 -> hpe-r1"
  echo "============================================================"
  ...
  echo
  echo "============================================================"
  echo "4. Final state before cleanup"
  echo "============================================================"
...
route_get_text = routeget_log.read_text(errors="ignore")
...
for line in route_get_text.splitlines():
    ...
    if "unreachable" in body.lower() or "Network is unreachable" in body:
...
rows = [
    "timestamp,test_name,alternate_path_disabled,old_next_hop,bgp_non_established_seen_ms,old_next_hop_seen_after_restart,alternate_next_hop_seen_after_restart,unreachable_seen_after_restart,stale_or_grace_text_seen,ping_tx,ping_rx,ping_lost,ping_loss_percent,bgp_non_established_line,missing_ping_sequences",
    f"{ts},bgp_gr_isolated_peer_restart,yes,{old_nh},{bgp_non_established_ms},{old_nh_seen_after_restart},{alternate_nh_seen_after_restart},{unreachable_seen_after_restart},{stale_or_grace_seen},{tx},{rx},{lost},{loss_percent},\"{bgp_non_established_line}\",\"{missing_text}\""
]
