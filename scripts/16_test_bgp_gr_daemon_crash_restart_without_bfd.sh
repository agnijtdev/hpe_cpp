...
EVIDENCE="evidence/bgp_daemon_restart/bgp_gr_daemon_crash_restart_without_bfd_${TS}.txt"
PING_LOG="evidence/bgp_daemon_restart/ping_bgp_gr_daemon_crash_restart_without_bfd_${TS}.log"
ROUTE_LOG="evidence/bgp_daemon_restart/route_bgp_gr_daemon_crash_restart_without_bfd_${TS}.log"
STATE_LOG="evidence/bgp_daemon_restart/state_bgp_gr_daemon_crash_restart_without_bfd_${TS}.log"
CSV="results/bgp_daemon_restart/bgp_gr_daemon_crash_restart_without_bfd_${TS}.csv"
LATEST="results/bgp_daemon_restart/bgp_gr_daemon_crash_restart_without_bfd.csv"
...
{
echo "============================================================"
echo "CONTROLLED BGP GR DAEMON CRASH/RESTART WITHOUT BGP-BFD"
echo "Timestamp: $TS"
echo "============================================================"
...
(
    ...
    while true; do
        ...
        RETURN_ROUTE=$(docker exec hpe-r9 ip route get 10.0.61.2 2>/dev/null | head -1)
        BIRD_ROUTE=$(docker exec hpe-r1 birdc show route 10.0.93.0/24 2>/dev/null | grep -m1 "10.0.93.0/24" || true)
        echo "${ELAPSED} ms | forward hpe-r1->hpe-h3: ${ROUTE} | return hpe-r9->hpe-h1: ${RETURN_ROUTE} | bird: ${BIRD_ROUTE}"
        sleep 0.05
    done
) > "$ROUTE_LOG" &
ROUTE_PID=$!
...
RETURN_ROUTE_MISSING_COUNT=$(grep "return hpe-r9->hpe-h1:" "$ROUTE_LOG" | grep -c "Network is unreachable\\|RTNETLINK\\|unreachable\\|throw\\|blackhole" || true)
RETURN_ROUTE_PRESENT_COUNT=$(grep "return hpe-r9->hpe-h1:" "$ROUTE_LOG" | grep -c "via" || true)
...
echo "Forward route missing/unreachable samples: $MISSING_ROUTE_COUNT"
echo "Return route present samples on hpe-r9: $RETURN_ROUTE_PRESENT_COUNT"
echo "Return route missing/unreachable samples on hpe-r9: $RETURN_ROUTE_MISSING_COUNT"
...
echo "$TS,bgp_gr_full_bird_daemon_crash_restart,hpe-r9,bgp_bfd_temporarily_disabled,$BIRDC_READY_MS,$BGP_REESTABLISHED_MS,${TX:-NA},${RX:-NA},${LOSS:-NA},$DIRECT_R9_COUNT,$ALT_R2_COUNT,$MISSING_ROUTE_COUNT,$FORWARDING_PRESERVED"
...
grep "^$TS,bgp_gr_full_bird_daemon_crash_restart" "$EVIDENCE" >> "$CSV"
cp "$CSV" "$LATEST"
