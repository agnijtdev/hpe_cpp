...
BASE_DIR="$PROJECT_DIR/generated_configs/tls_ao_mrt_loop_${N}"
RESULT_DIR="$PROJECT_DIR/results/mrt_tls_ao_loop_${N}_${PREFIX_COUNT}_announce${DELAY_MS}_delay${LINK_DELAY_MS}"
PREFIX_FILE="$BASE_DIR/exabgp/prefixes.txt"

CERT_DIR="$PROJECT_DIR/certs/tls_ao_mrt_loop_${N}"
...
network_name() {
  echo "bgpost_mrt_tls_ao_link_$1"
}

subnet_for_link() {
  echo "172.39.$((10 + $1)).0/24"
}
...
INJ_NET="bgpost_mrt_tls_ao_injector"
INJ_SUBNET="172.39.0.0/24"
EXABGP_IP="172.39.0.10"
R1_EXABGP_IP="172.39.0.2"

RETURN_NET="bgpost_mrt_tls_ao_return"
RETURN_SUBNET="172.39.20.0/24"
R10_RETURN_IP="172.39.20.11"
R1_RETURN_IP="172.39.20.12"
...
docker network ls --format '{{.Name}}' | grep '^bgpost_mrt_tls_ao_' | xargs -r docker network rm >/dev/null || true
...
write_bgp_block() {
  ...
  local low="$local_router"
  local high="$peer_router"
  ...
  local ao_password="PWD_r${low}-r${high}_PWD"
...
echo "[7/10] Copying BIRD configs and TLS-AO certificates..."
...
echo
echo "TLS-AO MRT loop experiment complete."
