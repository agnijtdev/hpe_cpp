...
OUT_DIR = PROJECT / f"results/final_5_mode_graphs_{COUNT}_announce{ANNOUNCE}_delay{LINK_DELAY}"
...
if missing:
    ...
    for label, path in missing:
        print(f"{label}: {path}")
    raise SystemExit("\nParse the missing MRT files first.")
...
plt.ylabel("CDF")
plt.title("BGP Prefix Propagation Duration — 15000 Prefixes, 25 ms Interval")
plt.grid(True, alpha=0.3)
plt.legend()
plt.tight_layout()

cdf_path = OUT_DIR / "cdf_5_modes_15000_announce25.png"
...
plt.title("BGP Prefix Propagation Distribution — 15000 Prefixes, 25 ms Interval")
plt.grid(True, axis="y", alpha=0.3)
plt.xticks(rotation=15, ha="right")
plt.tight_layout()

boxplot_path = OUT_DIR / "boxplot_5_modes_15000_announce25.png"
plt.savefig(boxplot_path, dpi=300)
plt.close()


print("\nFinal 15000-prefix 25ms graph generation complete.")
