...
def left_ip(link_id: int) -> str:
    return f"172.35.{link_subnet_octet(link_id)}.11"


def right_ip(link_id: int) -> str:
    return f"172.35.{link_subnet_octet(link_id)}.12"
...
protocol bgp to_r{peer_router} {{
    description "QUIC BGP session to r{peer_router}";
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python3 scripts/generate_quic_line_configs.py <number_of_routers>")
        print("Example: python3 scripts/generate_quic_line_configs.py 10")
        sys.exit(1)
    ...
    out_dir = Path(f"generated_configs/quic_line_{total}")
    out_dir.mkdir(parents=True, exist_ok=True)

    topology_lines = []
    topology_lines.append(f"QUIC line topology with {total} routers")
    ...
    for link_id in range(1, total):
        topology_lines.append(
            ...
            f"on 172.35.{link_subnet_octet(link_id)}.0/24"
        )
    ...
    print(f"Generated QUIC configs for {total} routers in {out_dir}")
