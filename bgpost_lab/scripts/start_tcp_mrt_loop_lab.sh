...
BASE_DIR="$PROJECT_DIR/generated_configs/tcp_mrt_loop_${N}"
RESULT_DIR="$PROJECT_DIR/results/mrt_tcp_loop_${N}_${PREFIX_COUNT}_announce${DELAY_MS}_delay${LINK_DELAY_MS}"
PREFIX_FILE="$BASE_DIR/exabgp/prefixes.txt"

mkdir -p "$BASE_DIR" "$BASE_DIR/exabgp" "$RESULT_DIR"

network_name() {
  echo "bgpost_mrt_tcp_link_$1"
}

subnet_for_link() {
  echo "172.36.$((10 + $1)).0/24"
}
...
INJ_NET="bgpost_mrt_tcp_injector"
INJ_SUBNET="172.36.0.0/24"
EXABGP_IP="172.36.0.10"
R1_EXABGP_IP="172.36.0.2"

RETURN_NET="bgpost_mrt_tcp_return"
RETURN_SUBNET="172.36.20.0/24"
R10_RETURN_IP="172.36.20.11"
R1_RETURN_IP="172.36.20.12"
...
docker network ls --format '{{.Name}}' | grep '^bgpost_mrt_tcp_' | xargs -r docker network rm >/dev/null || true

echo "[2/10] Generating prefixes..."
rm -f "$PREFIX_FILE"
...
for i in $(seq 1 "$N"); do
...

  {
    ...
    if [ "$i" -eq 1 ]; then
      ...
      echo 'mrtdump extended_timestamp on;'
      echo
    fi
  ...
  if [ "$i" -eq 1 ]; then
    write_bgp_block "exabgp" "$R1_EXABGP_IP" "$asn" "$EXABGP_IP" "65000" "    passive on;" "all" "none" >> "$BASE_DIR/r$i/bird.conf"
    write_bgp_block "to_r2" "$(left_ip 1)" "$asn" "$(right_ip 1)" "$((asn + 1))" "" "none" "all" >> "$BASE_DIR/r$i/bird.conf"
    write_bgp_block "to_r10" "$R1_RETURN_IP" "$asn" "$R10_RETURN_IP" "$((65000 + N))" "    passive on;" "all" "none" >> "$BASE_DIR/r$i/bird.conf"

  elif [ "$i" -eq "$N" ]; then
    left_link=$((i - 1))
    write_bgp_block "to_r$((i - 1))" "$(right_ip "$left_link")" "$asn" "$(left_ip "$left_link")" "$((asn - 1))" "    passive on;" "all" "none" >> "$BASE_DIR/r$i/bird.conf"
    write_bgp_block "to_r1" "$R10_RETURN_IP" "$asn" "$R1_RETURN_IP" "65001" "" "none" "all" >> "$BASE_DIR/r$i/bird.conf"
    ...
    write_bgp_block "to_r$((i - 1))" "$(right_ip "$left_link")" "$asn" "$(left_ip "$left_link")" "$((asn - 1))" "    passive on;" "all" "none" >> "$BASE_DIR/r$i/bird.conf"
    write_bgp_block "to_r$((i + 1))" "$(left_ip "$right_link")" "$asn" "$(right_ip "$right_link")" "$((asn + 1))" "" "none" "all" >> "$BASE_DIR/r$i/bird.conf"
  fi
done
...
echo "[7/10] Copying BIRD configs..."
...
echo
echo "TCP MRT loop experiment complete."
