...
echo "[replica-b] Binding anycast address ${ANYCAST_IP}/32 to lo..."
ip addr add ${ANYCAST_IP}/32 dev lo || echo "[replica-b] (already bound, continuing)"

echo "[replica-b] Starting NSD..."
nsd-checkconf /etc/nsd/nsd.conf
nsd -c /etc/nsd/nsd.conf

echo "[replica-b] Validating BIRD config syntax..."
bird -c /etc/bird/bird.conf -p

echo "[replica-b] Starting BIRD (BGPoTLS) in background so we can layer the healthcheck on top..."
...
echo "[replica-b] Starting healthcheck monitor (DNS liveness -> BGP withdraw/announce)..."
